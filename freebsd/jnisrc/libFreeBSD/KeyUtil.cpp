...
#ifndef AUTO_DEP
#include "javah/com_lightcrafts_platform_freebsd_FreeBSDKeyUtil.h"
#endif
...
#define FreeBSDKeyUtil_METHOD(method) \
        name4(Java_,com_lightcrafts_platform_freebsd_FreeBSDKeyUtil,_,method)
 ...
 * the ASCII code.)
 */
JNIEXPORT jboolean JNICALL FreeBSDKeyUtil_METHOD(isKeyPressed)
    ( JNIEnv *env, jclass, jint keyCode )
{
    ...
    if (display == NULL) {
        cerr << "FreeBSDPlatform cannot connect to X server "
             << XDisplayName(NULL)
             << endl;
        return false;
    }
