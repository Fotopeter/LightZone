This subversion directory (freebsd/jre) holds the custom-built JRE bundle that is
used by LightZone for FreeBSD.  For the freebsd build to work, this JRE bundle must
...
get the patch into the bundle is:
    ...
    cd $LIGHTZONE_HOME/freebsd/jre
    mkdir temp
    cd temp
    tar zxf ../freebsd-x86-1.6.0_03-lightzone.tar.gz
    cd lib
    ...
    rm -rf temp
    cd ..
    tar zcf ../freebsd-x86-1.6.0_03-lightzone.tar.gz *
    cd ..
    rm -rf temp
